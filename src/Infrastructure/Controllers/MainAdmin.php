...
class MainAdmin extends Controller
{
    public function actionIndex(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $discord = new DiscordClient(['token' => $config->settings->token]); // Token is required
        $guildMember = $discord->guild->getGuildMember(['guild.id' => (int)$config->settings->guildId, 'user.id' => 215036376654020608]);
